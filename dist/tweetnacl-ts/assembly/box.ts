...
export function box_open(msg: ByteArray, nonce: ByteArray, publicKey: ByteArray, secretKey: ByteArray): ByteArray | null {
    const k = box_before(publicKey, secretKey);

    return secretbox_open(msg, nonce, k);
}

export const box_open_after: (box: ByteArray, nonce: ByteArray, key: ByteArray) => ByteArray | null = secretbox_open;
...
export function box_keyPair_fromSecretKey(secretKey: ByteArray): BoxKeyPair {
    ...
    return new BoxKeyPair(pk, secretKey);
}

// low level
function _box_keypair(y: ByteArray, x: ByteArray):i32 {
    _randomBytes(x, 32);

    return _scalarMult_base(y, x);
}

function _box_beforenm(k: ByteArray, y: ByteArray, x: ByteArray):void {
    const s = ByteArray(32);

    _scalarMult(s, x, y);

    _hsalsa20(k, _0, s, _sigma);
}
